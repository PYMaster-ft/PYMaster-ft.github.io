...
if [ ! -z $VM ] && [ ! -z $OS ] && [ ! -z $LEVEL ]; then
    ...
    # 1. Create img folder
    mkdir -p ~/workspace/pymaster-ft.github.io/assets/img/htb/machines/$OS/$LEVEL/$LILVM
    # 2. Copy screenshots into img folder
    cp $HTB/machines/$MAJOS/$VM/img/*.png ~/workspace/pymaster-ft.github.io/assets/img/htb/machines/$OS/$LEVEL/$LILVM
    cp $HTB/machines/$MAJOS/$VM/img/*.jpeg ~/workspace/pymaster-ft.github.io/assets/img/htb/machines/$OS/$LEVEL/$LILVM 2>/dev/null
    cp $HTB/machines/$MAJOS/$VM/img/*.jpg ~/workspace/pymaster-ft.github.io/assets/img/htb/machines/$OS/$LEVEL/$LILVM 2>/dev/null
    # 3. Create WU post
    prefix=`date --rfc-3339=date`;
    wu_file="$HOME/workspace/pymaster-ft.github.io/_posts/$prefix-htb-$LILVM.md";
    echo "---" > $wu_file;
