...
if [ ! -z $VM ] && [ ! -z $OS ] && [ ! -z $OS ]; then
    ...
    # 1. Create img folder
    mkdir -p ~/workspace/pymaster-ft.github.io/assets/img/thm/$OS/$LILVM
    # 2. Copy screenshots into img folder
    cp $THM/$VM/img/*.png ~/workspace/pymaster-ft.github.io/assets/img/thm/$OS/$LILVM
    # 3. Create WU post
    prefix=`date --rfc-3339=date`;
    wu_file="$HOME/workspace/pymaster-ft.github.io/_posts/$prefix-thm-$LILVM.md";
    echo "---" > $wu_file;
